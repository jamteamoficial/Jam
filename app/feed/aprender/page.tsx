...
import { APRENDER_POSTS, type MockPost } from '@/app/data/mockPosts'

export default function AprenderFeed() {
  const { user } = useAuth()
  const { toast } = useToast()
  const [currentPosts, setCurrentPosts] = useState<MockPost[]>(APRENDER_POSTS)

  // Cargar publicaciones del usuario desde localStorage
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const userPosts = JSON.parse(localStorage.getItem('userPosts') || '[]')
      // Filtrar solo posts del feed aprender
      const aprenderUserPosts = userPosts.filter((post: MockPost) => 
        post.feedType === 'aprender'
      )
      if (aprenderUserPosts.length > 0) {
        // Combinar posts del usuario con los mock posts
        setCurrentPosts([...aprenderUserPosts, ...APRENDER_POSTS])
      }
    }
