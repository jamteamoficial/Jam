...
import { DESCUBRIR_POSTS, type MockPost } from '@/app/data/mockPosts'

export default function DescubrirFeed() {
  const { user } = useAuth()
  const { toast } = useToast()
  const [currentPosts, setCurrentPosts] = useState<MockPost[]>(DESCUBRIR_POSTS)

  // Cargar publicaciones del usuario desde localStorage
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const userPosts = JSON.parse(localStorage.getItem('userPosts') || '[]')
      // Filtrar solo posts del feed descubrir
      const descubrirUserPosts = userPosts.filter((post: MockPost) => 
        post.feedType === 'descubrir'
      )
      if (descubrirUserPosts.length > 0) {
        // Combinar posts del usuario con los mock posts
        setCurrentPosts([...descubrirUserPosts, ...DESCUBRIR_POSTS])
      }
    }
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 py-8">
                ...
                <div className="relative p-6 pt-20">
                  ...
                  <div className="flex items-center gap-3">
                    <Button
                      ...
                      className="flex-1 text-white font-bold py-3 rounded-xl shadow-lg transition-all hover:scale-105 hover:opacity-90"
                    ...
                    <Button
                      variant="outline"
                      className="bg-white px-4 py-3 rounded-xl hover:opacity-90"
      ...
      <button
        ...
        className="fixed bottom-8 right-8 w-20 h-20 text-white rounded-full shadow-2xl flex items-center justify-center text-4xl font-bold transition-all duration-300 hover:scale-110 active:scale-95 hover:opacity-90 z-50"
        style={{ backgroundColor: 'var(--rolex)' }}
